...
const Depose = () => {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3001/api/deposes')
      .then(response => setDeposes(response.data))
      .catch(error => console.error(error));
  }, []);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post('http://localhost:3001/api/deposes', newDepose);
      setDeposes([...deposes, response.data]);
      handleClose();
    } catch (error) {
      console.error(error);
    }
    ...
    <div style={{ backgroundColor: theme.colors.background, minHeight: '100vh', padding: theme.spacing.md }}>
      ...
      <Paper sx={{ width: '100%', overflow: 'hidden', mt: theme.spacing.md }}>
        <Table>
          ...
          <TableBody>
            {deposes.map((depose) => (
              <TableRow key={depose._id}>
                <TableCell>{new Date(depose.date).toLocaleString()}</TableCell>
                <TableCell>{depose.installation}</TableCell>
                <TableCell>{depose.matricule}</TableCell>
                <TableCell>{depose.puissance}</TableCell>
                <TableCell>{depose.vitesse}</TableCell>
                <TableCell>{depose.marque}</TableCell>
                <TableCell>{depose.voltage}</TableCell>
                <TableCell>{depose.isolementStat}</TableCell>
                <TableCell>{depose.isolementRot}</TableCell>
                <TableCell>{depose.observation}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
